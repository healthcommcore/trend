	...
	<table class="admintable">
        ...
        <tr>
			...
			<td>
            	...
            	<p><a class="modal" rel="{handler: 'iframe', size: {x: 750, y: 600}}" href="index.php?option=com_jce&tmpl=component&type=group&task=addusers"><span class="icon-add"><span class="icon-text"><?php echo JText::_('Add');?></span></span></a>
            	<a onclick="javascript: jce.removeSelect(document, 'users');" href="#"><span class="icon-remove"><span class="icon-text"><?php echo JText::_('Remove');?></span></span></a></p>
			</td>
        </tr>
    </table>
	</fieldset>
    ...
    <legend><?php echo JText::_( 'GROUPS LAYOUT' ); ?></legend>
        <table class="admintable">
            ...
            <tr>
                ...
                <fieldset>
                <legend><?php echo JText::_( 'GROUP LAYOUT' ); ?></legend>
        ...
        <table class="admintable">
            ...
            <tr>
				...
				foreach( $this->plugins as $plugin ){
					if( $plugin->layout == '' && $plugin->editable ){?>
						<tr>
...
<div class="col width-40">
	...
	<?php
		...
		echo $pane->startPanel(JText :: _('Standard'), "param-page");
	...
	<?php	
		foreach( $this->plugins as $plugin ){
			if($plugin->editable){
				...
				$xmlPath 	= JPATH_PLUGINS .DS. 'editors' .DS. 'jce' .DS. 'tiny_mce' .DS. 'plugins' .DS. $plugin->name . DS . $plugin->name .'.xml';
				$name 		= trim( $plugin->name ); 
				$params 	= new JParameter( $this->group->params, $xmlPath );
				// Load Language fro plugin
			...
			?>
                    <fieldset class="adminform">
                    <legend><?php echo JText::_( $plugin->title.' Parameters' ); ?></legend>
            <?php
                    $pane =& JPane::getInstance('sliders');
