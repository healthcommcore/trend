...
class plgEditorJCE extends JPlugin
{
	...
	function onInit(){
		...
		$load = "\t<script type=\"text/javascript\" src=\"". $url ."plugins/editors/jce/tiny_mce/tiny_mce.js?". $jce->getVersion() ."\"></script>\n";
		$load .= "<script type=\"text/javascript\" src=\"". $url ."plugins/editors/jce/libraries/js/editor.js?". $jce->getVersion() ."\"></script>\n";
		...
		if( $jce->checkUser() ){			
			...
			$param['theme_advanced_blockformats'] 			= $jce->getParam( $params, 'editor_theme_advanced_blockformats', 'p,div,h1,h2,h3,h4,h5,h6,blockquote,dt,dd,code,samp,pre', 'p,div,h1,h2,h3,h4,h5,h6,blockquote,dt,dd,code,samp,pre' );
			
			$param['theme_advanced_fonts']					= $jce->getEditorFonts( $jce->getParam( $params, 'editor_theme_advanced_fonts_add', '' ), $jce->getParam( $params, 'theme_advanced_fonts_remove', '' ) );	
			...
			if( $params->get( 'editor_content_css', 1 ) == 0 ){
				$param['content_css'] = $url . str_replace( '$template', $jce->getSiteTemplate(), $params->get( 'editor_content_css_custom', '' ) );
			}	
		...
		JContentEditor.set({
			...
			allowToggle : ". $params->get( 'editor_toggle', '0' ) .",
