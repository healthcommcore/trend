...
class JContentEditorPlugin extends JContentEditor {
	...
	*/
	function getPluginParams(){				
		...
		if( $this->_group ){
			$g_params = $this->filterParams( $this->_group->params, 'plugin' );
		}	
				
		$query = "SELECT params FROM #__jce_plugins"
		. "\n WHERE name = '" . $this->_plugin->name . "'"
		. "\n AND published = 1" 
		. "\n LIMIT 1";
	...
	*/
	...
	function getUploadAction(){		
		...
		return JURI::base() .'index.php?option=com_jce&task=plugin&plugin=' . $this->_plugin->name . '&file=' . $file .'&action=upload'; 
	}
	/**
	...
	function printCss(){
		...
		$browser = $browser = &JBrowser::getInstance();
		//$stamp 		= $this->_debug ? '?'.time() : '';
		$stamp		= '?'. $this->_version;
