...
class JContentEditor extends JObject{
	...
	*/
	function getUserGroup(){
		...
		foreach( $groups as $group ){
			if( in_array( $this->_id, explode(',', $group->users ) ) ){
				return $group;
			}else if( in_array( $this->_gid, explode(',', $group->types ) ) ){
				return $group;
			}
		}
		return null;
	...
	*/
	...
	function trimParamKey( $var ){
		return substr( $var, strpos( $var, '_' ) + 1 );
	}
	function filterParams( $params ){	
		return implode( "\n", array_filter( explode( "\n", $params ), array( $this, 'trimParamKey' ) ) ); 
	}
	/**
	...
	*/
	function getEditorParams(){		
		...
		if( $this->_group ){
			$g_params = $this->filterParams( $this->_group->params );
		}
	...
	*/
	function getPluginParams( $plugin ){				
		...
		. "\n AND published = 1" 
		. "\n LIMIT 1";
		$p_params = $this->_query( $query, 'loadResult' );
				
		return new JParameter( $p_params . $g_params );
	}
	/**
	...
	*/
	function getPlugins(){		
		if( $this->_group ){			
			...
			;
			if( !$group = $this->_query( $query, 'loadResultArray' ) ){
				$group = array();
			}
